...
if __name__ == '__main__':
    ...
    train_loader, val_loader = get_cifar_loader(
        ...
        val_split=0.1, random_seed=int(args.seed)
    )
    test_loader = get_cifar_loader(
        train=False, batch_size=int(args.batch_size) * 2, num_workers=int(args.num_workers)
    )
    ...
    print("\n🚀 开始训练...")
    losses_list, train_loss, val_loss, train_acc, val_acc, best_val_acc = train(
        model=model,
        optimizer=optimizer,
        criterion=criterion,
    ...
    with torch.no_grad():
        final_test_acc = get_accuracy(model, test_loader, device)
        final_test_loss = get_loss(model, test_loader, criterion, device)

    save_experiment_config(exp_metrics, args, exp_id)

    config_path = os.path.join(exp_metrics, "config.txt")
    with open(config_path, "a", encoding="utf-8") as f:
        f.write(f"Final Test Accuracy: {final_test_acc:.2f}%\n")

    save_experiment_results(
        exp_metrics, args, losses_list, 
    ...
    print("\n" + "=" * 60)
    print("🎉 实验完成！")
    print(f"最佳验证准确率: {best_val_acc:.2f}% (Epoch {checkpoint['epoch'] + 1})")
    print(f"✅ 最终官方测试准确率: {final_test_acc:.2f}%")
    print(f"最终官方测试损失: {final_test_loss:.4f}")
