...
def main():
    ...
    for exp_name in sorted(os.listdir(reports_root)):
        ...
        try:
            best_acc = float(config.get('Best Validation Accuracy', '0').replace('%', ''))
        except:
            best_acc = 0.0
        ...
        exp_info = {
            ...
            'best_acc': config.get('Best Validation Accuracy', 'unknown'),
    ...
    experiments.sort(key=lambda x: x['best_acc_val'], reverse=True)

    md = "# 实验结果汇总\n\n"
    md += "| ID | 模型 | 学习率 | 激活 | 优化器 | 最佳准确率 | 参数量 | 说明 |\n"
    md += "|---|---|---|---|---|---|---|---|\n"

    for e in experiments:
        md += f"| {e['id']} | {e['model']} | {e['lr']} | {e['activation']} | {e['optimizer']} | {e['best_acc']} | {e['params']} | {e['note']} |\n"
